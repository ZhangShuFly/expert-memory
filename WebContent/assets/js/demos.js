$(function () {
  ...
  var  avatar = new Array("http://dwz.cn/5tWBbb","http://dwz.cn/5tWCy2","http://dwz.cn/5tWDSV");
  var  name = new Array("李靖瑶","张欣妍","王梦洁");
  var  date = new Array("2016-10-6","2016-11-8","2016-10-9");
  
  var num = 0;
  //下拉刷新页面
  $(document).on("pageInit", "#indexPage", function(e, id, page) {
    var $content = $(page).find(".content").on('refresh', function(e) {
      // 模拟2s的加载过程
      setTimeout(function() {
    	 num = num+1;
    	 if(num >= avatar.length){
    		 num = 0;
    	 }
        var cardHTML = 
          '<div class="card facebook-card">' +
          	'<div class="card-header no-border">' +
          		'<div class="facebook-avatar"><img src="'+avatar[num]+'" width="34" height="34"></div>' +
          		'<div class="facebook-name">'+name[num]+'</div>' +
          		'<div class="facebook-date">'+date[num]+'</div>'+
            '</div>' +
            '<div class="card-content"><img src="'+avatar[num]+'" width="100%"></div>'+
        ...
        $content.find('.content-block').prepend(cardHTML);
        // $(window).scrollTop(0);
        // 加载完毕需要重置
  ...
  //操作表
  $(document).on("pageInit", "#settingPage", function(e, id, page) {
